...
def _prune_dims(variances, threshold=0.01):
    """Mask for dimensions collapsed to the prior."""
    scale_z = np.sqrt(variances)
    return scale_z >= threshold

def compute_factor_vae(ground_truth_data,representation_function,random_state,batch_size,num_train,num_eval,num_variance_estimate):
    ...
    eval_votes = _generate_training_batch(ground_truth_data, representation_function, batch_size, num_eval,
...
def _compute_variances(ground_truth_data, representation_function, batch_size,
    ...
    representations,_ = representation_function.encoder(Variable(torch.from_numpy(observations)).cuda(device_id))
